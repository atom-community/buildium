...
[![License](https://img.shields.io/github/license/idleberg/atom-buildium?color=blue&style=for-the-badge)](https://github.com/idleberg/atom-buildium/blob/master/LICENSE)
[![Release](https://img.shields.io/github/v/release/idleberg/atom-buildium?style=for-the-badge)](https://github.com/idleberg/atom-buildium/releases)
[![Downloads](https://img.shields.io/pulsar/dt/buildium?style=for-the-badge&color=slateblue)](https://web.pulsar-edit.dev/packages/buildium)
...
A build file in your home directory acts as a fallback for every project. All of the names above are recognised there too, so a `~/.buildium.yaml` supplies its targets to any project that has no build file of its own — and to those that do, since the targets of both are shown together.
...
| `functionMatch`   | `(Function \| Function[])?` | **Interpreted formats only**. A function, or list thereof, that returns a list of match objects                                                                                                                                                   |
...
| `preBuild`        | `Function?`                 | **Interpreted formats only**. Called _before_ executing `cmd`, with no arguments. `this` is the build configuration.                                                                                                                              |
| `postBuild`       | `Function?`                 | **Interpreted formats only**. Called _after_ executing `cmd`, with three arguments: `boolean buildOutcome`, `string stdout` and `string stderr`. `this` is the build configuration.                                                               |
...
| `{FILE_ACTIVE_NAME_BASE}`     | Base name of the active file, e.g. `buildium`                                        |
...
| <kbd>Esc</kbd>                                                                | Terminates build and closes the build panel |
