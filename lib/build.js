...
const activePath = () => {
  ...
  return atom.project.getPaths().sort((a, b) => b.length - a.length).find(p => {
    try {
      ...
      return fs__default['default'].realpathSync(textEditor.getPath()).substr(0, realpath.length) === realpath;
    } catch (err) {
...
function getConfig(file) {
  ...
  switch (path__default['default'].extname(file)) {
    ...
    case '.cson':
      return CSON__default['default'].parse(fs__default['default'].readFileSync(realFile));

    case '.yaml':
    case '.yml':
      return YAML__default['default'].safeLoad(fs__default['default'].readFileSync(realFile));
  }
...
class CustomFile extends EventEmitter__default['default'] {
  ...
  isEligible() {
    this.files = [].concat.apply([], ['json', 'cson', 'yaml', 'yml', 'js'].map(ext => [path__default['default'].join(this.cwd, `.atom-build.${ext}`), path__default['default'].join(os__default['default'].homedir(), `.atom-build.${ext}`)])).filter(fs__default['default'].existsSync);
